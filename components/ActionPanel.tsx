...
import { Loader2, Zap, CheckCircle, AlertCircle, Wand2, Type, Sparkles, Minimize2, Settings2, Target, Info, Download, ChevronDown, ChevronUp, Scissors } from 'lucide-react';
...
export const ActionPanel: React.FC<ActionPanelProps> = ({ mode, fileCount, onMerge, onDownloadAll, status, error, fileName, onFileNameChange, namingContext, onNamingContextChange, compressionLevel, onCompressionLevelChange, targetSize, onTargetSizeChange, metadata, onMetadataChange, splitRange, onSplitRangeChange }) => {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-4">
        <div className="flex items-center gap-3 text-sm text-gray-600 font-medium"><Info className="w-5 h-5 text-blue-500" /> {mode === AppMode.MERGE ? `Merging ${fileCount} assets into one file` : mode === AppMode.SPLIT ? `Extracting from document` : `Optimizing ${fileCount} PDFs`}</div>
        <div className="flex gap-4 w-full sm:w-auto">
          ...
          <button onClick={onMerge} disabled={isProcessing} className={`px-12 py-5 rounded-[1.5rem] font-black text-lg flex items-center gap-3 transition-all shadow-2xl active:scale-95 ${isSuccess ? 'bg-blue-100 text-blue-600' : isProcessing ? 'bg-gray-400 text-white' : 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white hover:shadow-blue-200'}`}>{isProcessing ? <><Loader2 className="w-6 h-6 animate-spin" /> Processing...</> : isSuccess ? <><CheckCircle className="w-6 h-6" /> Complete</> : <>{mode === AppMode.MERGE ? <Zap className="w-6 h-6 fill-current" /> : mode === AppMode.SPLIT ? <Scissors className="w-6 h-6" /> : <Minimize2 className="w-6 h-6" />} {mode === AppMode.MERGE ? 'Process & Export' : mode === AppMode.SPLIT ? 'Split & Export' : 'Optimize All'}</>}</button>
        </div>
      </div>
    </div>
  );
